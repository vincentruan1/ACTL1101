...
graph1 <-
  ggplot(data = perc_ind_death, aes(x = perc_ind_death$PercInd, y = perc_ind_death$Std)) + geom_point(color =
                                                                                                        alpha("black")) + geom_smooth(method = "lm", color = alpha("red")) + labs(
                                                                                                          title = "Percentage of Aboriginal people vs Standardised Death Rates",
                                                                                                          x = "Percentage of Aboriginal people",
...
graph1 + annotate(
  ...
  x = 30,
  y = 13,
  label = "Gradient of regression line = 2.836927",
  colour = "blue",
  size = 5
...
names(Population) <-
  c(
    ...
    "Fertility Rate",
    "Deaths",
    "Standarised Death Rate per 1000",
    "delete",
    "Percentage of Aborignals"
  )

Population$delete <- NULL

Population$`Percentage of Aborignals` <-
  as.numeric(Population$`Percentage of Aborignals`)

Population$`Fertility Rate` <-
  as.numeric(Population$`Fertility Rate`)

Population$`Deaths` <- as.numeric(Population$`Deaths`)

Population$`Standarised Death Rate per 1000` <-
  as.numeric(Population$`Standarised Death Rate per 1000`)

Population <- Population[complete.cases(Population), ]

plot(
  log(Population$`Percentage of Aborignals`),
  Population$`Fertility Rate`,
  xlab = "Log Of Percentage of Aboriginals",
  ylab = "Fertility Rate",
  main = "Fertility Rate vs Aboriginal Percentage Rate"
)
